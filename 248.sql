    ...
    (
        SELECT
            item_info.INVENTORY_ITEM_ID               ONE_INVENTORY_ITEM_ID
                ...
                --                 OEM org  SOH
                 , SUM(
                CASE WHEN  orders.ORDER_TYPE = 18 and '7371_IO_GWFB_OEM'=:P_ITEM_ORG
                     THEN  orders.ORDER_QUANTITY
                     ELSE  0
                END
                )  as   ONE_SOH_OEM
...
--                 OEM  this week  TD
                , SUM(
                CASE WHEN  (orders.ORDER_TYPE = 1001 or  orders.ORDER_TYPE = 1003 or orders.ORDER_TYPE = 1082 and
                '7371_IO_GWFB_OEM'=:P_ITEM_ORG )
                     THEN  orders.ORDER_QUANTITY
                     ELSE  0
                END
                )  as   ONE_TD_OEM
--                   Bejing this week TD
                    , SUM(
                CASE WHEN  (orders.ORDER_TYPE = 1001 or  orders.ORDER_TYPE = 1003 or orders.ORDER_TYPE = 1082 and
                '7379_IO_GWFB_BJ'=:P_ITEM_ORG )
                     THEN  orders.ORDER_QUANTITY
                     ELSE  0
                END
                )  as   ONE_TD_BJ
                --                   hongkong this week TD
                    , SUM(
                CASE WHEN  (orders.ORDER_TYPE = 1001 or  orders.ORDER_TYPE = 1003 or orders.ORDER_TYPE = 1082 and
                :P_ITEM_ORG  in ('7351_IO_WW','7361_IO_CGI') )
                     THEN  orders.ORDER_QUANTITY
                     ELSE  0
                END
                )  as   ONE_TD_HK
...
--             AND orders.ORDER_TYPE IN( 18,1,1001,1003,1082)
     ...
     future_one_week-- 对应  I  J  库存和 在途     M  12  本周预测
    as
    (
        SELECT
            item_info.INVENTORY_ITEM_ID               ONE_INVENTORY_ITEM_ID
...
--                 )  as   ONE_TD
        ...
        WHERE
            ...
            -- oders   filter
--             AND (orders.FIRM_STATUS = 'Firm' or orders.ORDER_TYPE IN( 18,1,1001))
            AND orders.ORDER_TYPE = 5
            AND orders.PLANNED_ORDER_TYPE  IN('Buy','购买')
--             AND orders.ORDER_TYPE IN( 1001,1003,1082)

            AND to_char(orders.SUGGESTED_START_DATE , 'YYYY-MM-DD')  IN
                                                        (    SELECT to_char(CALENDAR_DATE,'YYYY-MM-DD') set_time
            FROM MSC_ANALYTIC_CALENDARS_V
            where WEEK_START_DATE =  (select WEEK_START_DATE
            from MSC_ANALYTIC_CALENDARS_V
...
--             AND orders.ORDER_TYPE IN( 18,1,1001,1003,1082)
    ...
    (
        ...
        , week_avg.AVG_WEEK 
        , future_one.ONE_SOH
        , future_one.ONE_SOH_OEM     ONE_SOH_OEM

        , future_one.ONE_SPO 
--         , future_one.ONE_PPO
        , future_one.ONE_TD 
        , future_one.ONE_TD_OEM       ONE_TD_OEM
        , future_one.ONE_TD_BJ        ONE_TD_BJ
        , future_one.ONE_TD_HK        ONE_TD_HK

--         future_one_week
        , future_one_week.ONE_PPO        ONE_PPO_WEEK
...
--         , future_one_period.ONE_SOH      FOUR_SOH
--         , future_one_period.ONE_SPO      FOUR_SPO
--         , future_one_period.ONE_PPO      FOUR_PPO
        , future_one_period.ONE_TD       ONE_PERIOD_TD


        , future_two_period.ONE_TD       TWO_PERIOD_TD

        FROM mast_item , week_avg, future_one ,future_one_week, future_two , future_three ,
         future_one_period ,future_two_period
        WHERE
       --left join  week_avg
...
--             left join  one  week
            and mast_item.INVENTORY_ITEM_ID      = future_one_week.ONE_INVENTORY_ITEM_ID(+)
            and mast_item.SUPPLIER_SITE_ID       = future_one_week.ONE_SUPPLIER_SITE_ID(+)
            ...
            and mast_item.INVENTORY_ITEM_ID      = future_one_period.ONE_INVENTORY_ITEM_ID(+)
            and mast_item.SUPPLIER_SITE_ID       = future_one_period.ONE_SUPPLIER_SITE_ID(+)
--             left join  future_two_period
            and mast_item.INVENTORY_ITEM_ID      = future_two_period.ONE_INVENTORY_ITEM_ID(+)
            and mast_item.SUPPLIER_SITE_ID       = future_two_period.ONE_SUPPLIER_SITE_ID(+)
...
SELECT 
        ...
        , all_order_tabl.AVG_WEEK 
        , all_order_tabl.ONE_SOH
        , all_order_tabl.UOM                                                             UOM
        , all_order_tabl.LEAD_TIME                                                             LEAD_TIME
        , all_order_tabl.SHELF_LIFE_DAYS                                                       SHELF_LIFE_DAYS
        , all_order_tabl.MIN_QTY                                                               MIN_QTY
        , all_order_tabl.ONE_SOH_OEM

        , all_order_tabl.ONE_SPO 
        , all_order_tabl.ONE_TD 
        , all_order_tabl.ONE_TD_OEM
        , all_order_tabl.ONE_TD_BJ
        , all_order_tabl.ONE_TD_HK

--         all_order_tabl_week
        , all_order_tabl.ONE_PPO_WEEK

        , all_order_tabl.TWO_TD

        , all_order_tabl.THRE_TD

        , all_order_tabl.ONE_PERIOD_TD


        , all_order_tabl.TWO_PERIOD_TD
...
--     this  view  just  supplier site code  mapping  (for mrp subject area)
    tab_b as (
